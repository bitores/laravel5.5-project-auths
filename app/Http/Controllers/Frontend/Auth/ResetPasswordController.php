...
class ResetPasswordController extends Controller
{
     ...
     * @return array
     */
    public function resetByPhone(ResetPasswordRequest $request)
    {
        $user = $this->user->findByPhone($request->get('mobile'));
        $user->password = bcrypt($request->get('password'));

        $user->save();

        return redirect()->route(homeRoute())->withFlashSuccess('重置成功,请登录');
    }
}
